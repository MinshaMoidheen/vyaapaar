...
export default function AddPurchaseReturnPage() {
  ...
  const updatePaymentType = (id: string, field: keyof PaymentTypeRow, value: string) => setPaymentTypes(prev => prev.map(p => p.id === id ? { ...p, [field]: value } : p))
  const addPaymentType = () => setPaymentTypes(prev => [...prev, { id: `p${Date.now()}`, type: 'Select Type', amount: '0', referenceNo: '' }])
  const removePaymentType = (id: string) => setPaymentTypes(prev => (prev.length > 1 ? prev.filter(p => p.id !== id) : prev))
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Input value={phoneNo} onChange={(e) => setPhoneNo(e.target.value)} placeholder="Enter phone number" className="mt-1 w-64" />
                </div>
              </div>
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input value={returnNo} onChange={(e) => setReturnNo(e.target.value)} className="mt-1" />
                  </div>
                  <div>
                    <Label className="text-sm font-medium">Bill Number</Label>
                    <Input value={billNo} onChange={(e) => setBillNo(e.target.value)} className="mt-1" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
        ...
        <div className="flex justify-between items-start">
          <div className="flex flex-col space-y-3">
              ...
              {paymentTypes.map((payment, index) => (
                <div key={payment.id} className="space-y-2">
                  <div className="flex items-center space-x-3">
                    ...
                    <Input value={payment.amount} onChange={(e) => updatePaymentType(payment.id, 'amount', e.target.value)} placeholder="Amount" className="flex-1" />
                    {paymentTypes.length > 1 && (
                      <Button variant="ghost" size="sm" onClick={() => removePaymentType(payment.id)} className="h-8 w-8 p-0 text-red-600"><X className="h-4 w-4" /></Button>
                    )}
                  </div>
                  {index > 0 && (
                    <div className="flex items-center space-x-3">
                      <Input value={payment.referenceNo} onChange={(e) => updatePaymentType(payment.id, 'referenceNo', e.target.value)} placeholder="Reference No." className="w-32" />
            ...
            <div className="space-y-2">
              ...
              <input id="pr-image-upload" type="file" accept="image/*" onChange={handleImageUpload} className="hidden" />
          ...
          <div className="flex items-center space-x-6">
            <div className="flex items-center space-x-2">
              <Checkbox id="round-off" checked={roundOff} onCheckedChange={(checked) => setRoundOff(checked as boolean)} />
              <Label htmlFor="round-off" className="text-sm">Round Off</Label>
              <Input value={roundOffValue} onChange={(e) => setRoundOffValue(e.target.value)} className="w-16 h-8" disabled={!roundOff} />
            </div>
            <div className="flex items-center space-x-2">

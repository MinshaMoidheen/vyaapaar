...
export default function EditSaleReturnPage() {
	 ...
	 useEffect(() => {
		 ...
		 const loadReturnData = () => {
			 ...
			 setCustomerName('Minsha')
			 setPhoneNo('+1 234 567 8900')
			 setRefNo('CN-001')
			 setReturnDate('2024-01-15')
			 setDueDate('2024-01-30')
			 setStateOfSupply('same')
			 setReturnReason('defective')
			 setDescription('Sample return description')
			 setTotal('290.00')
	 ...
	 const updateItem = (id: number, field: keyof ReturnItemRow, value: string) => {
		 setItems(items.map(item => 
			 item.id === id ? { ...item, [field]: value } : item
		 ))
	 }

	 const calculateAmount = (item: ReturnItemRow) => {
	 ...
	 return (
		 <div className="min-h-screen bg-gray-50">
			 ...
			 <div className="flex items-center justify-between">
				 <div className="flex items-center space-x-4">
					 ...
					 <span className="text-sm font-medium">Edit Sale Return - {refNo}</span>
				 </div>
				 <div className="flex items-center space-x-4">
		 ...
		 <div className="grid grid-cols-2 gap-8 ps-6">
